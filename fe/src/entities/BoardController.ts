...
import {
    ...
    PlayerColor,
    SvgBoardStates,
    DocumentClickData, GameProgressUpdate, FigureDataset, GameProgressDataset, SvgElementsLayers, GameElementsDict, SvgElements
} from "../types";
import App from "../App";
...
import { GameElement } from "./svg/GameElement";
...
export class BoardController {
    ...
    public setSvgProgress(progress: GameProgressDataset) {
        ...
        PlayersOrder.forEach((color: PlayerColor) => {
            for (let i = 0; i < 4; i++) {
    ...
    private init() {
        ...
        const startFields = {} as Record<PlayerColor, Field[]>
        const homeFields = {} as Record<PlayerColor, Field[]>
        const figures = {} as Record<PlayerColor, Figure[]>
    ...
    private getFreeStartField(color: PlayerColor) {
